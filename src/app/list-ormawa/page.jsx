...
export default function ListOrmawaPage() {
  ...
  function storageUrl(path) {
    ...
    // kalau backend sudah kasih URL lengkap
    if (s.startsWith("http://") || s.startsWith("https://")) return s;

    // kalau backend kasih "/storage/...."
    if (s.startsWith("/storage/")) return `${API_BASE}${s}`;

    // kalau backend kasih path yang diawali "/"
    if (s.startsWith("/")) return `${API_BASE}${s}`;

    // kalau backend simpan "public/ormawa/xxx.jpg"
    const cleaned = s.replace(/^public\//, "");

    // default: "ormawa/xxx.jpg"
          ...
          {!loading &&
            filteredOrmawa.map((o) => (
              <CardOrmawa
                ...
                imageUrl={storageUrl(o.photo_path) || "/placeholder.png"}
              />
            ))}
